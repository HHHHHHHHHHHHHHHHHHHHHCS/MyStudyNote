...
![](Images/MobileHardware_06.jpg)
  ...
  当然也有例外的时候, 比如说场景被UI遮挡的部分和场景被手遮挡的部分都不会显得那么重要. 这时候就引入了 **Metal Variable Rate Rasterization Rate Map**.
...
第二行图RRM, 左边一半为原始分辨率, 右边一半为1%的分辨率. 在渲染的时候左边占了绝大部分, 但是upscale回去比例是正常的, 不过右边明显模糊了.

第三行图RRM, 使用和苹果tech talk demo推荐比例, 中间大周围小.
...
面前市面上只有adreno660以上机型支持, 如: 小米11.

mali和麒麟芯片的内部人员介绍, 目前feature已经开发完毕, 但是芯片还没上市.

### **3.3 使用**

左面是针对ui抠出来一个mask图, 把mask图作为创建subpass的输入参数, 使该subpass在绘制对应tile的时候使用相应的vrs级别.

右边图是目前ue在editor中已经添加好的功能, 每个材质球都有可以设置该材质球的vrs级别的属性.
...
如果没有开AsyncCompute, 可以看到lightgrid和shadowmap中间UC你在gap.
...
而且看图, 这两个CS已经并行计算了, 同时还有PS在同时执行.

同时现在的游戏都是多线程渲染比较多, 当前帧绘制时, 下一帧的CPU和CommnadBuffer可能都已经准备好了.

看上图, 绿色表示下一帧, 黄色表示当前帧数, 需要等黄色全部执行完毕后才能继续执行.

但是下图, 蓝色表示下一帧, 蓝色的一些准备工作已经提前到黄色的当前帧来执行了.
...
除了trace标记让GPU管理依赖之外, metal还有四种资源同步方法.
...
如图, 当一个wavefront中出现了两种分支都会涉及的情况, 则会出现很gpu核心空转, 影响了gpu利用率, 增加耗时.
...
上图 如果把lightgrid, shadowmap, mainpass放在一个commandbuffer中, 当这么大的commandbuffer的cpu执行结束后, gpu才开始执行.

下图, 把lightgrid+shadowmap拆除一个单独的commandbuffer, mainpass为另外一个commandbuffer. 在gpu中, lightgrid和shadowmap的计算提前. 虽然cpu耗时没有变, 但latency缩短, 一帧的耗时就减少了.
