...
{
	// 触手的prefab
	private Tentacle tantaclePrefab;
...
public class Mimic : MonoBehaviour
{
	...
	private void ResetMimic()
	{
		...
		tantaclePrefab = useMesh ? tentacleMesh : tentacleBillboard;
	}
}
...
private void RequestTentacle(Vector3 legPosition, int resolution, float _maxLegDistance, float growCoef, float lifeTime)
{
	...
	{
		newTentacle = Instantiate(tantaclePrefab, transform.position, Quaternion.identity);
	}
...
它原来的代码还有一种 **LineRenderer** 面片渲染模式. 但是我这里删除, 只保留了比较复杂的Mesh渲染, 所以只使用 **LineRenderer** 中的points 和 width.
