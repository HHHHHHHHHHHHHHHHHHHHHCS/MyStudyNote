...
## &emsp;&emsp; 不知道取什么标题... 本文应该有很多问题,欢迎斧正.
...
![AndroidBlitType](Images/AndroidBlitType.png)
...
![https://www.imgtec.com/blog/a-look-at-the-powervr-graphics-architecture-tile-based-rendering/](Images/EarlyZ.png)
...
![https://www.expreview.com/24705-3.html](Images/IMR.png)

&emsp;&emsp; 但是Mobile平台因为功耗问题，不能使用这套简单粗暴的管线，所以就使用了另一套———TBR（Tile-Based Rendering，基于瓦片渲染）。

![https://www.expreview.com/24705-3.html](Images/TBR.png)
![http://cdn.imgtec.com/sdk-documentation/PowerVR+Series5.Architecture+Guide+for+Developers.pdf](Images/ParameterBuffer.png)
...
&emsp;&emsp; 在手机，TBR虽然比IMR能解决很大部分的功耗问题，但是还有进一步的优化空间——TBDR（Tile Based Deferred Rendering，贴图延迟渲染）。它可以用储存在中间的FrameData做一些优化。如PowerVR/IOS的HSR技术和ARM Mail的FPK技术。大体作用是尽可能找到最后看到的fragment shader，剔除掉多余的从而减少Overdraw。如对于不透明的，一些ztest失败，或者stencil失败的物体，在HSR的阶段被kill，根本不会进入fragment shader阶段。

![https://docs.imgtec.com/PowerVR_Architecture/topics/powervr_architecture_hidden_surface_removal_efficiency.html](Images/TBDR.png)
![https://www.imgtec.com/blog/a-look-at-the-powervr-graphics-architecture-tile-based-rendering/](Images/HSR.png)
...
![https://www.jianshu.com/p/5a99750327fc?utm_campaign=maleskine&utm_content=note&utm_medium=seo_notes&utm_source=recommendation](Images/FPK.jpg)
...
4、TBDR的MSAA带宽消耗不增加。PC的IMR上因为是对整块FBO进行操作，所以是有很大的带宽消耗。但是在TBDR下MSAA不会有额外的带宽消耗。如把一块tile拷贝到SRAM，进行4Xmsaa，最后resolving成1X，写回到DRAM，带宽并没有增加。
