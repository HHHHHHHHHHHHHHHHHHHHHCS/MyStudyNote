...
- [**3. 自定义顶点工厂**](#3-自定义顶点工厂)
  ...
  - [**3.3. UMyMeshComponent**](#33-umymeshcomponent)
  - [**3.4. FMyMeshSceneProxy**](#34-fmymeshsceneproxy)
  - [**3.5. FMyMeshVertexFactory**](#35-fmymeshvertexfactory)
  - [**3.6. 依赖库**](#36-依赖库)
- [**4. Buffer**](#4-buffer)
  - [**4.1. MyMeshShaderCommon**](#41-mymeshshadercommon)
  - [**4.2. MyMeshBuffer**](#42-mymeshbuffer)
...
打开UE 顶部栏 Edit -> Plugins -> 左上角 Add -> Editor Tool Button, 我这里起名为 MyVertexTool. (起名有点过于随意了, UE 的 Plugins 创建流程里可选择的模板太多了)
...
尝试编译, UE 启动! 其实还可以进一步规范 Editor 结构和代码, 这里偷懒不展开细讲, 如果需要可以看下下面几个的修改.
...
&emsp;&emsp; 先尝试创建界面, 再用界面点击按钮打开保存窗口, 生成 `StaticMesh` 储存到本地, 最后用自定义 `CustomNode` 用于 Pixel 渲染.
...
&emsp;&emsp; 上面的流程和外部创建模型后再导入的区别不大. 接着就是自定义顶点工厂. 支持动态修改顶点, Draw Instance 等. 但是实现起来也比较麻烦.
