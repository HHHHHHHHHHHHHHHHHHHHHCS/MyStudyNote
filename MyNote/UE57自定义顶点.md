...
- [**4. Buffer与Shader接入**](#4-buffer与shader接入)
  ...
  - [**4.3. 插件加载阶段**](#43-插件加载阶段)
- [**5. 渲染绘制**](#5-渲染绘制)
  - [**5.1. 完善 VertexFactory**](#51-完善-vertexfactory)
...
现在可以编译通过了, 但是启动时会报错. 大致意思是 Vertex Factory 加载需要在引擎初始化之后.
...
现在就只剩下启动的时候 Shader 的编译报错了.

------------------------

## **5. 渲染绘制**

现在处于 **Shader 编译报错** 和 **渲染绘制** 都没有完善的地步. 我这里决定先修改 **渲染绘制** 相关代码. 因为 Shader 的代码比较冗长.

### **5.1. 完善 VertexFactory**
...
再来看 **ShouldCompilePermutation()** , **ModifyCompilationEnvironment()**  和 **ValidateCompiledResult()**.
...
**ValidateCompiledResult()** : 编译后决定 Shader 是否合格. 不合格就报错/警告并拒绝生成. 比如说在 GBuffer 阶段或 RayTracing 阶段去读取 SceneTexture. 在比如说当前编译完成的 Shader 只支持 UniformBuffer 和 LooseData(CBuffer).
...
这个 Demo Vertex Input 只有 float3 position. 如果使用了 GPU Scene , 就把 instance index 放在 Attribute 13 (Default) 或 Attribute 1 (PositionOnly/PositionAndNormalOnly).
...
`InitResource` 和 `ReleaseResource` Vertex Buffer. 确认 Vertex Stream 布局 和 Attribute Index 对应的属性.
