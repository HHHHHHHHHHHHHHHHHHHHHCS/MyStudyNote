...
在AMD YES在GDC 2008中有一篇文章 [文章地址][15] 介绍了PCF的时候对当前点添加自适应的偏移值, 来解决shadow acne的小问题.
...
有了上面的半径之后, 再次进行PCF计算出阴影. 这里为了图简单, 复用泊松盘PCF计算. 
...
做法:
  + 当我们保存shadowmap的时候, 不是写入简单的depth, 而是写入exp(C*depth). 记得去掉ShadowBias不然效果可能会错.
...
为了防止除以0 (深度都一样的情况), 需要给一个保护. 

当o2不变的时候, diff差距越大, 即遮挡和投影物体的深度差距越大, 值越接近于0.  不过如果投影物体不写入深度, 这时候去看投影物体的表面效果会是错的.
...
&emsp;&emsp; SDF的具体可以参考IQ大佬的网站. SDF具体讲挺复杂的, 单生成SDF 3DTexture就可以单独写一篇.
...
我们看到的position, 沿着光照方向做步进. 如果检测到了则为这个点为阴影.

![](Images/EasyShadow_50.jpg)

### **7.2 做法**

制作SDF图的时候, 需要记录Size. 因为要把物体压缩到UVW 0~1的3D贴图, 后面还要再还原AABB回来.

单拿一个物体来说, 我们要传入物体的WorldToLocalMatrix 和 SDF的Extent.
...
可以先用RayCastBox让射线是否会击中物体的AABB进行预测, 如果没有命中就不用进行下面的SDF步进了. 因为rayStart可能在AABB的里面, 所以RayCastBox得到的两个值中的一个为负数, 让其强制为0 为起点.
...
这里没有讲CSM. CSM有时候也比较复杂. 因为他可以远处用ESM, 近处用PCF. 还有CSM Blend. 还有更新频率等.

Wiki上还写了一堆别的shadowmap的相关文章 [Wiki地址][22].

NVDIA 在 GDC 上也有一篇不错的shadow PPT [文章地址][21].

-----------------

已知奥日史低为17R, XBOX手柄为750R. 玩完之后 残破的XBOX+1, 没有通关游戏+1, 啊这.
